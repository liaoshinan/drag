<template>
  <div class="index">
    <div class="function-list">
      <div v-for="(item, index) in funcArry" :key="index" @click="choseFun(index)" class="fun-name">
        {{ item }}
      </div>
    </div>
    <div id="father" class="canvas-box">
      <svg xmlns="http://www.w3.org/2000/svg" id="svg" version="1.1" height="600" width="1000">
        <defs>
          <marker id="idArrow" viewBox="0 0 20 20" refX="20" refY="10" markerUnits="strokeWidth" markerWidth="5" markerHeight="13" orient="auto">
            <path d="M 0 0 L 20 10 L 0 20 z" fill="#eee" stroke="#eee" />
          </marker>
        </defs>
      </svg>
      <div v-for="(item, index) in choseArry" :key="index" v-if="item !== null" class="block" @dblclick="deleteBox(index, $event)">
        <div class="point-box">
          <span class="point point-top1"> </span>
          <span class="point point-top2"> </span>
          <span class="point point-top3"> </span>
          <span class="point point-bottom1"> </span>
          <span class="point point-bottom2"> </span>
          <span class="point point-bottom3"> </span>
        </div>
        {{ item }}
      </div>
    </div>
  ...
  methods: {
    deleteBox(i, event) {
      ...
      for (let i = 0; i < deleteDom.length; i++) {
        let startId = deleteDom[i].getAttribute('data-start') ? deleteDom[i].getAttribute('data-start').split(',') : []
        let endId = deleteDom[i].getAttribute('data-end') ? deleteDom[i].getAttribute('data-end').split(',') : []
        if (startId[0]) {
      ...
      this.$set(this.choseArry, i, null)
      ...
      document.onmouseup = function(evn) {
        if (_this.currentLine !== '') {
          if (evn.target.classList[0] !== 'point') {
            ...
            let startId = _this.startPointDom.getAttribute('data-start') ? _this.startPointDom.getAttribute('data-start').split(',') : []
            let endId = _this.endPointDom.getAttribute('data-end') ? _this.endPointDom.getAttribute('data-end').split(',') : []
    ...
    queryLine(domArry, boxLeft, boxTop) {
      for (let i = 0; i < domArry.length; i++) {
        ...
        let endId = domArry[i].getAttribute('data-end') ? domArry[i].getAttribute('data-end').split(',') : []
        if (startId[0]) {
          for (let j = 0; j < startId.length; j++) {
            let lineDom = document.getElementById(startId[j])
